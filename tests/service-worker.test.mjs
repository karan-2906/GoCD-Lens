...
globalThis.chrome = {
  runtime: {
    ...
    async sendMessage(message) {
      ...
      return audioBlocked
        ? { ok: false, error: "blocked by autoplay policy" }
        : { ok: true };
    },
  },
  storage: {
...
function dashboardBody(viewName = null) {
  ...
  const visible = Object.entries(world).filter(
    ([name]) => !allowed || allowed.has(name),
  );
  return {
    _embedded: {
      pipeline_groups: [
        { name: "core", pipelines: visible.map(([name]) => name) },
      ],
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (path === "/go/api/internal/pipeline_selection") {
    return json({
      filters: [
        { name: "Mine", type: "whitelist", state: [], pipelines: ["web-app"] },
      ],
    });
  }
...
function send(type, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (kept !== true)
      reject(new Error(`handler for ${type} did not keep the channel open`));
  });
}
...
test("the state a page receives never contains the credential", async () => {
  ...
  assert.ok(
    ...
    `the token reached a page: ${serialised}`,
  );
...
test("the first load is silent, so connecting does not fire a wall of alerts", async () => {
  ...
  assert.deepEqual(
    calls.notifications,
    [],
    "there is no previous state to have changed from",
  );
});
...
test("a pipeline turning red notifies once, and staying red stays quiet", async () => {
  ...
  assert.equal(
    ...
    1,
    "notify on the change, not on the state",
  );
});
...
test("a starred pipeline is alerted on, an unstarred one is not", async () => {
  ...
  assert.deepEqual(
    calls.notifications,
    [],
    "api is neither starred nor watched",
  );
...
test("the starred alert can be switched off on its own", async () => {
  ...
  assert.deepEqual(
    calls.notifications,
    [],
    "starring alone no longer interrupts",
  );
});
...
test("turning notifications off keeps the badge but stops the popups", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "1",
    "the badge is still the quiet signal",
  );
});
...
test("once the cache is stale the ETag is sent, so an unchanged server costs a 304", async () => {
  ...
  assert.equal(
    again.pipelines.length,
    2,
    "a 304 must still answer with the cached data",
  );
  const dashboardRequest = requests.find((r) =>
...
test("when GoCD is unreachable the cache is served, flagged as stale", async () => {
  ...
  assert.equal(
    ...
    2,
    "the last good data stays browsable",
  );
  assert.match(result.error, /VPN|permission|Cannot reach/);
  assert.equal(
    calls.badgeText.at(-1),
    "!",
    "and the badge says the data is old",
  );
});
...
test("editing a connection without retyping the token keeps the stored one", async () => {
  ...
  assert.equal(
    storage.get("connection").keepExistingSecret,
    undefined,
...
test("switching auth mode does not carry the old secret across", async () => {
  ...
  assert.equal(
    stored.token,
    undefined,
    "a token must not linger behind a mode that does not use one",
  );
});
...
test("diagnostics separate the API from the web UI", async () => {
  ...
  assert.match(
    report.probes.find((p) => p.id === "dashboard").detail,
    /2 pipelines/,
  );
});
...
test("a pipeline being re-run reads as running, not as failing", async () => {
  ...
  assert.equal(
    ...
    "#2563EB",
    "it sat red with nothing to count",
  );
...
test("a search typed into the popup narrows the badge with it", async () => {
  ...
  assert.match(
    calls.titles.at(-1),
    /matching "web"/,
    "a narrowed badge has to say so",
  );
...
test("the badge can be told to follow your watch list", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "1",
    "api is failing too, but is not watched",
  );
...
test("running outranks failing on the badge", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "1",
    "the one that is running, not the one that is red",
  );
  assert.equal(calls.badgeColor.at(-1), "#2563EB");
  assert.match(
    calls.titles.at(-1),
    /1 running, 1 failing/,
    "and the tooltip still reports both",
  );
});
...
test("picking a view while a poll is in flight is not answered with the view you left", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "",
    "the badge kept counting the view you left",
  );
});
...
test("selecting a view narrows what the server sends back", async () => {
  ...
  assert.equal(
    mine.pipelines.length,
    1,
    "GoCD filters it, so fewer pipelines arrive",
  );
...
test("the chosen view survives a refresh that does not mention one", async () => {
  ...
  assert.equal(
    polled.view,
    "Mine",
    'omitting the view must mean "keep it", not "reset it"',
  );
...
test("the chosen view is remembered across restarts", async () => {
  ...
  assert.equal(
    ...
    "Mine",
    "a page reload should not lose the selection",
  );
});
...
test("switching views does not fire notifications for pipelines that merely reappeared", async () => {
  ...
  assert.deepEqual(
    calls.notifications,
    [],
    "coming back into view is not a change of state",
  );
});
...
test("the badge follows the active view, because that is what you are watching", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "1",
    "only the view\u2019s pipelines are counted",
  );
});
...
test("a failure says so, names the stage, and stays on screen", async () => {
  ...
  assert.match(
    alert.message,
    /build/,
    "the stage that went red is worth naming",
  );
  assert.equal(
    alert.requireInteraction,
    true,
    "a failure should not scroll past unseen",
  );
...
test("a run that starts and finishes between two polls is still reported", async () => {
  ...
  assert.equal(
    calls.notifications.length,
    1,
    "the new run counter is the only evidence left",
  );
  assert.match(calls.notifications[0].title, /passed/i);
});
...
test("the badge source can be pinned to watching or starred explicitly", async () => {
  ...
  assert.match(
    calls.titles.at(-1),
    /of 1 starred/,
    "a setting change repaints at once",
  );
...
test("a test notification is sent for real, because the OS cannot be asked", async () => {
  ...
  assert.deepEqual(
    calls.sounds,
    ["success"],
    "and it makes the sound a real one would",
  );
});
...
test("a test notification is silent when chimes are off, like a real one", async () => {
  ...
  assert.deepEqual(
    calls.sounds,
    [],
    "but it does not make a noise you did not ask for",
  );
});
...
test("a run parked at an approval gate is not counted as running", async () => {
  ...
  assert.equal(
    calls.badgeText.at(-1),
    "",
    "nothing is running and nothing is failing",
  );
});
