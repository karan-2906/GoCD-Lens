          ...
          instances: [
            {
              counter: 42,
              label: "42",
              _embedded: { stages: [{ name: "build", status: "Failed" }] },
            },
          ],
        },
      },
  ...
  server = http.createServer((req, res) => {
    ...
    if (path === "/go/api/dashboard") {
      if (req.headers["if-none-match"] === "etag-1")
        return json(res, 304, "", { ETag: "etag-1" });
    ...
    if (path === "/go/api/pipelines/web-app/history") {
      ...
      return json(res, 200, {
        _links: after
          ? {}
          : { next: { href: "/go/api/pipelines/web-app/history?after=41" } },
        pipelines: [
          {
            name: "web-app",
            counter: after ? 41 : 42,
            ...
            stages: [
              {
                ...
                counter: "1",
                jobs: [
                  { name: "compile", result: "Failed", state: "Completed" },
                ],
              },
            ],
          },
    ...
    if (
      req.method === "GET" &&
      /^\/go\/api\/stages\/web-app\/42\/integration\/\d+$/.test(path)
    ) {
      return json(res, 200, {
        name: "integration",
    ...
    if (
      req.method === "POST" &&
      path === "/go/api/pipelines/web-app/schedule"
    ) {
      let body = "";
    ...
    if (
      req.method === "POST" &&
      /\/go\/api\/pipelines\/web-app\/(pause|unpause)$/.test(path)
    ) {
      let body = "";
      req.on("data", (chunk) => (body += chunk));
      return req.on("end", () => {
    ...
    if (
      req.method === "POST" &&
      /^\/go\/api\/stages\/web-app\/42\/build\/1\/(cancel|run-failed-jobs)$/.test(
        path,
      )
    ) {
    ...
    if (
      req.method === "POST" &&
      path === "/go/api/stages/web-app/42/build/run"
    ) {
      return json(res, 202, { message: "accepted" });
    }
    ...
    if (
      req.method === "POST" &&
      path === "/go/api/stages/web-app/42/build/1/run-selected-jobs"
    ) {
      let body = "";
      req.on("data", (chunk) => (body += chunk));
      return req.on("end", () => {
    ...
    if (
      path === "/go/files/web-app/42/build/1/compile/cruise-output/console.log"
    ) {
      const start = Number(url.searchParams.get("startLineNumber") || 0);
      const lines = [
        "in|10:00:00.000 one",
        "in|10:00:01.000 two",
        "in|10:00:02.000 three",
      ];
    ...
    if (path === "/go/api/version")
      return json(res, 200, { version: "23.5.0" });
    if (path === "/go/api/current_user")
      return json(res, 200, { login_name: "karan" });

    if (path === "/go/api/pipelines/missing/history")
      return json(res, 404, { message: "Pipeline not found" });
    if (path === "/go/api/pipelines/denied/history")
      return json(res, 401, { message: "not authorised" });
    if (path === "/go/api/pipelines/forbidden/history")
      return json(res, 403, { message: "no permission" });
...
const tokenClient = () =>
  new GoCdClient({ serverUrl: base, authMode: "token", token: "secret-token" });
const lastRequest = (pathStart) =>
  seen.filter((r) => r.url.startsWith(pathStart)).pop();
...
test("a trailing slash on the server URL does not double up in paths", async () => {
  ...
  assert.ok(
    lastRequest("/go/api/version"),
    "expected /go/api/version, not /go//api/version",
  );
});
...
test("a view request says an empty view really is empty", async () => {
  ...
  seen = [];
  await tokenClient().dashboard({ view: "Mine" });
  const params = new URL(lastRequest("/go/api/dashboard").url, "http://x")
    .searchParams;
...
test("view names with other awkward characters survive the round trip", async () => {
  for (const name of ["R&D", "team/platform", "a b  c", "caf\u00e9"]) {
    ...
    const { url } = lastRequest("/go/api/dashboard");
    assert.equal(
      new URL(url, "http://x").searchParams.get("viewName"),
      name,
      url,
    );
  }
...
test("history pages forward using the cursor from _links.next", async () => {
  ...
  assert.equal(
    first.next,
    41,
    "the next cursor comes out of the link, not a guess",
  );
...
test("mutating calls carry X-GoCD-Confirm and the right API version", async () => {
  ...
  assert.equal(
    schedule.headers["x-gocd-confirm"],
    "true",
    "GoCD rejects these without it",
  );
  ...
  const rerun = lastRequest(
    "/go/api/stages/web-app/42/build/1/run-failed-jobs",
  );
  assert.equal(
    rerun.headers.accept,
  ...
  assert.ok(
    run,
    "running a stage addresses the stage, not one of its attempts",
  );
  assert.equal(
    run.headers.accept,
    "application/vnd.go.cd.v2+json",
    "this one is v2, not v3",
  );
});

test("triggering with variables sends them and asks for fresh materials", async () => {
  seen = [];
  await tokenClient().trigger("web-app", [
    { name: "DEPLOY_ENV", value: "staging" },
  ]);
  const body = JSON.parse(
    lastRequest("/go/api/pipelines/web-app/schedule").body,
  );
...
test("pausing sends a reason and unpausing sends no body", async () => {
  ...
  assert.deepEqual(
    JSON.parse(lastRequest("/go/api/pipelines/web-app/pause").body),
    {
  ...
  assert.equal(
    unpause.headers["content-type"],
    undefined,
    "and so declares no content type",
  );
...
test("tailing asks only for the lines it has not already seen", async () => {
  const client = tokenClient();
  const whole = await client.consoleLog(
    "web-app",
    42,
    "build",
    "1",
  ...
  const tail = await client.consoleLog(
    "web-app",
    42,
    "build",
    "1",
  ...
  assert.match(
    tail,
    /three/,
    "the tail must resume where the last fetch stopped",
  );
});
...
test("a console log fetch at offset zero sends no cursor at all", async () => {
  ...
  const request = lastRequest(
    "/go/files/web-app/42/build/1/compile/cruise-output/console.log",
  );
  assert.ok(!request.url.includes("startLineNumber"));
  assert.ok(
    !request.headers.accept?.includes("vnd.go.cd"),
    "the file server is not the versioned API",
  );
});

test("an earlier attempt of a re-run stage is read from the stage instance", async () => {
  seen = [];
  const stage = await tokenClient().stageInstance(
    "web-app",
    42,
    "integration",
    "1",
  ...
  assert.equal(
    request.method,
    "GET",
    "reading an attempt must not be one of the run verbs",
  );
...
test("the attempt number is in the path, so attempts do not collapse into one", async () => {
  ...
  assert.equal(
    third.counter,
    3,
    "asking for attempt 3 must not answer with attempt 1",
  );
});

test("the artifact listing comes from the file server, not the JSON API", async () => {
  const tree = await tokenClient().artifacts(
    "web-app",
    42,
    "build",
    "1",
...
test("nothing in the client writes to an internal endpoint", async () => {
  ...
  assert.ok(
    internal.length > 0,
    "views does read one, so this test is proving something",
  );
  assert.deepEqual([...new Set(internal.map((r) => r.method))], ["GET"]);
  assert.equal(
    typeof client.saveView,
    "undefined",
    "the write was removed, not just unwired",
  );
});
...
test("a 200 that is not JSON is explained, not thrown as a parser crash", async () => {
  await assert.rejects(tokenClient().history("garbled"), (err) => {
    ...
    assert.match(
      err.message,
      /\/go/,
      "the hint should mention the usual cause",
    );
    return true;
  });
});
...
test("a dead host fails fast with something actionable", async () => {
  const dead = new GoCdClient({
    serverUrl: "http://127.0.0.1:1/go",
    authMode: "session",
  });
...
test("web UI links point at the pages GoCD actually serves", () => {
  ...
  assert.equal(
    client.webUrl("pipeline", parts),
    `${base}/pipeline/activity/web-app`,
  );
  assert.equal(
    client.webUrl("run", parts),
    `${base}/pipelines/value_stream_map/web-app/42`,
  );
  assert.equal(
    client.webUrl("stage", parts),
    `${base}/pipelines/web-app/42/build/1`,
  );
  assert.equal(
    client.webUrl("job", parts),
    `${base}/tab/build/detail/web-app/42/build/1/compile`,
  );
});
...
test("the history cursor is read from the link, and nonsense is ignored", () => {
  assert.equal(
    nextPageCursor("http://go/api/pipelines/x/history?after=205"),
    205,
  );
...
test("the host permission covers the whole origin, not just the /go path", () => {
  assert.equal(
    originPattern("https://gocd.example.com/go"),
    "https://gocd.example.com/*",
  );
  assert.equal(
    originPattern("http://localhost:8153/go/"),
    "http://localhost:8153/*",
  );
  assert.equal(
    originPattern("https://api.github.com"),
...
test("re-running selected jobs names them, so eleven passing jobs are left alone", async () => {
  ...
  seen = [];
  await tokenClient().rerunSelectedJobs("web-app", 42, "build", "1", [
    "zod",
    "canvas",
  ]);

  const request = lastRequest(
    "/go/api/stages/web-app/42/build/1/run-selected-jobs",
  );
  assert.equal(request.headers.accept, "application/vnd.go.cd.v3+json");
