...
class FakeNode {
  ...
  get elements() {
    return this.childNodes.flatMap((c) =>
      c instanceof FakeElement ? [c, ...c.elements] : [],
    );
  }
}
...
test("text given to el() stays one text node, whatever it contains", () => {
  for (const payload of PAYLOADS) {
    ...
    assert.equal(
      node.textContent,
      payload,
      "the text must survive byte for byte",
    );
    assert.equal(node.childNodes.length, 1);
    assert.ok(
      node.childNodes[0] instanceof FakeText,
      `${payload} was not kept as text`,
    );
    assert.deepEqual(
      node.elements,
      [],
      `${payload} created elements -- it was parsed as markup`,
    );
  }
});
...
test("handlers are attached as listeners, never as inline attributes", () => {
  ...
  assert.deepEqual(
    ...
    [],
    'no onclick="" attribute is written',
  );
...
test("search highlighting preserves the text exactly and only wraps the hits", () => {
  ...
  assert.equal(
    node.textContent,
    name,
    "highlighting must not lose or reorder a character",
  );
...
test("icons are built as SVG nodes pointing at the bundled sprite", () => {
  ...
  assert.equal(
    ...
    "#i-refresh",
    "a local sprite reference, not a remote URL",
  );
});
