...
const { applyView, isBuiltinView, BUILTIN_VIEWS } =
...
function pipeline(name, status, { paused = false } = {}) {
  return {
    ...
    _embedded: {
      instances: [
        { counter: 1, _embedded: { stages: [{ name: "build", status }] } },
      ],
    },
  };
}
...
test("a status-pinned view can ask for running or paused work", () => {
  ...
  assert.deepEqual(
    names(applyView(PAYLOAD, { ...base, state: ["building"] })),
    ["personalization-ui"],
  );
  assert.deepEqual(names(applyView(PAYLOAD, { ...base, state: ["paused"] })), [
    "auth-service",
  ]);
  assert.deepEqual(
...
test("the filter chips count only what is inside the view", () => {
  ...
  assert.deepEqual(
    running,
    [],
    "personalization-ui is running, but it is not in this view",
  );
  assert.equal(
    inView.pipelines.length,
    2,
    "and the All count is the view, not the instance",
  );
});
...
test("a built-in view is just a whitelist of what you picked", () => {
  ...
  assert.deepEqual(
    groupNames(result),
    ["cda"],
    "and only the groups those live in",
  );
});
