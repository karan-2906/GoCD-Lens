...
globalThis.chrome = {
  runtime: {
    sendMessage: async () => ({ ok: true, data: { groups: [], pipelines: [], view: null } }),
  },
};
...
function pipeline(name, status, { paused = false } = {}) {
  return {
    ...
    pause_info: { paused },
    _embedded: {
      instances: [{ counter: 1, label: '1', _embedded: { stages: [{ name: 'build', status }] } }],
    },
  };
}
...
test('a group match is marked as such, and never outranks a name match', () => {
  ...
  assert.equal(matches[0].pipeline.name, 'api-build-test', 'the name match comes first');
  assert.equal(matches[0].viaGroup, null);

  const viaGroup = matches.find((m) => m.pipeline.name === 'payments-nightly');
  assert.equal(viaGroup.viaGroup, 'api', 'and the group match is labelled so the row makes sense');
});
...
test('the search box and the filter chips narrow the same one list', () => {
  ...
  state.filter = 'failing';
  assert.deepEqual(names(), ['web-app-deploy'], 'they stack rather than replacing each other');
});
...
test('the sidebar filter does not touch which pipelines the list shows', () => {
  ...
  assert.equal(matchingPipelines().length, 4, 'it narrows the sidebar and nothing else');
});
...
test('a search typed against the list follows you into the new view', () => {
  ...
  assert.equal(state.group, null, 'but a group chosen in the old view may not be in this one');
...
test('watched names answer for the watched view', () => {
  ...
  assert.deepEqual(rememberedNames(), ['payments-nightly'], 'each view answers with its own list');
});
...
test('an empty view is not an outage', () => {
  ...
  assert.deepEqual(rememberedNames(), [], 'no error means the list is empty for an honest reason');

  state.loadError = 'not-configured';
  assert.deepEqual(rememberedNames(), [], 'an unconfigured extension is not a failed request');
});
...
test('pipelines that did load are always worth more than remembered names', () => {
  ...
  assert.deepEqual(rememberedNames(), [], 'a stale cache has real pipelines to show instead');
});
...
test('the search box still narrows the names during an outage', () => {
  ...
  state.search = 'wad';
  assert.deepEqual(rememberedNames(), ['web-app-deploy'], 'the same fuzzy match as the list');
