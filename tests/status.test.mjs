...
test('rollup reports the worst status across stages', () => {
  ...
  assert.equal(rollup(['Scheduled']), 'Building', 'scheduled work is work in flight');
});

test('rollup does not call a never-run pipeline green', () => {
  assert.equal(rollup([]), 'Unknown');
  assert.equal(rollup([undefined]), 'Unknown');
  assert.equal(rollup([null, undefined]), 'Unknown', 'nothing known about any stage');
});
...
test('dashboard entries and history entries roll up the same way', () => {
  const dashboardEntry = {
    _embedded: {
      instances: [{ _embedded: { stages: [{ status: 'Passed' }, { status: 'Failed' }] } }],
    },
  };
  assert.equal(pipelineStatus(dashboardEntry), 'Failed');
  assert.equal(runStatus({ stages: [{ status: 'Passed' }, { status: 'Failed' }] }), 'Failed');
});
...
test('a running job is reported by its state, not its stale result', () => {
  ...
  assert.equal(jobStatus({ state: 'Scheduled', result: 'Unknown' }), 'Scheduled');
...
test('a stage is read as running whichever field the endpoint used', () => {
  ...
  assert.equal(
    stageStatus({ name: 'build', jobs: [{ name: 'compile', state: 'Building', result: 'Unknown' }] }),
    'Building',
  );
  assert.equal(
    stageStatus({ name: 'build', jobs: [{ name: 'compile', state: 'Scheduled' }] }),
    'Building',
...
test('a finished stage is read from its jobs when no status field arrives', () => {
  const completed = (result) => ({ name: 'compile', state: 'Completed', result });
  assert.equal(stageStatus({ jobs: [completed('Passed'), completed('Passed')] }), 'Passed');
  assert.equal(stageStatus({ jobs: [completed('Passed'), completed('Failed')] }), 'Failed');
...
test('the run header agrees with the job that is still running', () => {
  ...
  const run = {
    stages: [
      { name: 'setup', status: 'Passed', jobs: [{ state: 'Completed', result: 'Passed' }] },
      {
        name: 'e2e-groups',
        status: 'Unknown',
        jobs: [
...
test('a run rolls up to Building from either endpoint shape', () => {
  const fromDashboard = { stages: [{ status: 'Passed' }, { status: 'Building' }] };
  const fromHistory = {
    stages: [
  ...
  assert.equal(runStatus(fromHistory), 'Building', 'the same run must not read differently once opened');
});
...
test('every status a view can be handed has a label and a tone', () => {
  ...
  const produced = ['Passed', 'Failed', 'Cancelled', 'Building', 'Scheduled', 'Unknown'];
  for (const status of produced) {
    assert.ok(STATUS[status], `${status} has no entry`);
    assert.ok(STATUS[status].label && STATUS[status].tone);
  }
  assert.equal(STATUS.Scheduled.tone, 'build', 'queued work should read as in-flight, not idle');
});
...
test('fuzzy search matches initials and reports where it matched', () => {
  ...
  assert.deepEqual(fuzzyMatch('', 'anything'), [], 'an empty query matches everything');
...
test('a contiguous match ranks above a scattered one', () => {
  assert.ok(matchScore('build', 'web-build') < matchScore('build', 'b-u-i-l-d-x'));
  assert.equal(matchScore('nope', 'web-app'), null);
});

test('git material descriptions parse in both SSH and HTTPS forms', () => {
  assert.deepEqual(parseGitMaterial('URL: git@github.com:acme/web-app.git, Branch: main'), {
    host: 'github.com',
    owner: 'acme',
    repo: 'web-app',
    branch: 'main',
  });
  assert.deepEqual(parseGitMaterial('URL: https://ghe.corp.io/platform/deploy, Branch: release'), {
    host: 'ghe.corp.io',
    owner: 'platform',
    repo: 'deploy',
    branch: 'release',
  ...
  assert.deepEqual(
    parseGitMaterial('URL: https://ghe.corp.io/platform/deploy.git, Branch: release'),
    { host: 'ghe.corp.io', owner: 'platform', repo: 'deploy', branch: 'release' },
  );
  assert.equal(parseGitMaterial('URL: git@github.com:acme/web.git').branch, 'main', 'branch defaults');
});
...
test('git refs come out of a build cause, skipping non-git materials', () => {
  const run = {
    build_cause: {
      material_revisions: [
        {
          material: { type: 'Git', description: 'URL: git@github.com:acme/web.git, Branch: main' },
          modifications: [{ revision: 'abc123', user_name: 'Dev <d@e.f>', comment: 'fix it' }],
        },
        { material: { type: 'Pipeline', description: 'upstream [ build ]' }, modifications: [] },
        {
          material: { type: 'Git', description: 'URL: https://ghe.corp.io/p/deploy, Branch: release' },
...
test('a revision with a hostile SHA is skipped', () => {
  const run = {
    build_cause: {
      material_revisions: [
        {
          material: { type: 'Git', description: 'URL: git@github.com:a/b.git, Branch: main' },
...
test('upstream dependencies are read from Pipeline material revisions', () => {
  ...
  const withDep = (revision) => ({
    build_cause: {
      material_revisions: [{ material: { type: 'Pipeline' }, modifications: [{ revision }] }],
    },
  });
  ...
  for (const bad of ['no-slashes', 'name/notanumber/stage/1', '/389/stage/1']) {
    assert.deepEqual(upstreamDeps(withDep(bad)), [], `${bad} should not yield a dependency`);
  }
  ...
  const tree = [
    {
      ...
      files: [
        ...
        { name: 'maps', type: 'folder', files: [{ name: 'a.map', type: 'file', url: 'https://x/a.map' }] },
      ],
    },
  ];

  assert.equal(flattenArtifacts(tree, new Set()).length, 1, 'a deep tree must not render all at once');
...
test('identically named folders under different parents keep separate state', () => {
  const branch = (name) => ({
    name,
    type: 'folder',
    files: [{ name: 'logs', type: 'folder', files: [{ name: 'a.txt', type: 'file', url: 'u' }] }],
  });
  const rows = flattenArtifacts([branch('one'), branch('two')], new Set(['one', 'one/logs', 'two']));
...
test('a node with children is a folder even without a type', () => {
  const rows = flattenArtifacts([{ name: 'untyped', files: [{ name: 'inner.txt' }] }], new Set());
  assert.equal(rows[0].isFolder, true, 'otherwise its children would be unreachable');
});
...
test('ANSI colouring from the build is stripped, not shown raw', () => {
  ...
  const framedAndColoured = parseLogLine(`er|10:00:00.000 ${ESC}[1;33mwarning: x${ESC}[0m`);
