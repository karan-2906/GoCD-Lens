...
function walk(dir, out = []) {
  for (const entry of readdirSync(dir)) {
    if (['.git', '.githooks', 'docs', 'node_modules', 'tests', 'tools'].includes(entry)) continue;
...
test('the manifest asks for no permission it does not use', () => {
  ...
  for (const dangerous of ['tabs', 'webRequest', 'cookies', 'history', 'bookmarks', 'downloads', 'scripting', 'debugger']) {
    assert.ok(!manifest.permissions.includes(dangerous), `${dangerous} is not needed`);
  }
});

test('there are no content scripts, so no web page can be read or touched', () => {
  assert.equal(manifest.content_scripts, undefined);
  assert.equal(manifest.web_accessible_resources, undefined, 'no page may reach into this extension either');
});

test('the content security policy blocks remote and inline code', () => {
  const csp = manifest.content_security_policy.extension_pages;
  assert.match(csp, /script-src 'self'/, 'no script from anywhere but this package');
  assert.match(csp, /object-src 'none'/);
  assert.match(csp, /frame-src 'none'/);
  assert.match(csp, /base-uri 'none'/);
  assert.match(csp, /form-action 'none'/);
  assert.ok(!csp.includes("'unsafe-inline'"), 'inline script would defeat the whole policy');
...
test('the service worker is a module and every entry point it names exists', () => {
  ...
  for (const path of referenced) {
    assert.ok(existsSync(join(ROOT, path)), `the manifest points at a missing file: ${path}`);
  }
});
...
test('nothing anywhere writes to Chrome’s syncing storage', () => {
  ...
  for (const file of SHIPPED.filter((f) => ['.js', '.html', '.json'].includes(extname(f)))) {
    assert.ok(!/storage\.sync/.test(code(file)), `${name(file)} touches chrome.storage.sync`);
  }
});
...
test('no source builds DOM from a string or evaluates one', () => {
  for (const file of byExtension('.js')) {
    ...
    assert.ok(!/insertAdjacentHTML/.test(text), `${name(file)} uses insertAdjacentHTML`);
    assert.ok(!/document\.write/.test(text), `${name(file)} uses document.write`);
    assert.ok(!/\beval\s*\(/.test(text), `${name(file)} calls eval`);
    assert.ok(!/new\s+Function\s*\(/.test(text), `${name(file)} builds a Function from a string`);
    assert.ok(!/setTimeout\s*\(\s*['"`]/.test(text), `${name(file)} passes a string to setTimeout`);
  }
});
...
test('no page loads a script, style, font or image from the network', () => {
  for (const file of byExtension('.html')) {
    ...
    assert.ok(!/(src|href)\s*=\s*["']https?:/i.test(text), `${name(file)} references a remote asset`);
    assert.ok(!/<script(?![^>]*\ssrc=)/i.test(text), `${name(file)} contains an inline script`);
    assert.ok(!/\son[a-z]+\s*=/i.test(text), `${name(file)} has an inline event handler attribute`);
  }
  for (const file of byExtension('.css')) {
    const text = read(file);
    assert.ok(!/@import\s+url\(\s*["']?https?:/i.test(text), `${name(file)} imports a remote stylesheet`);
    assert.ok(!/url\(\s*["']?https?:/i.test(text), `${name(file)} loads a remote resource`);
  }
});
...
test('the only host referenced anywhere is the user\u2019s own server', () => {
  ...
  for (const file of SHIPPED.filter((f) => ['.js', '.html'].includes(extname(f)))) {
    ...
    for (const match of text.matchAll(/https?:\/\/([a-z0-9.-]+)/gi)) {
      const host = match[1];
      const isExample = /(^|\.)example\.(com|org)$|^ghe\.example\.com$|^gocd\./.test(host);
...
test('every icon a view asks for is actually drawn in the sprite', () => {
  ...
  const drawn = new Set([...sprite.matchAll(/id="i-([a-z-]+)"/g)].map((m) => m[1]));

  const asked = new Set();
  for (const file of byExtension('.js')) {
    const text = read(file);
    for (const m of text.matchAll(/\bicon\(\s*'([a-z-]+)'/g)) asked.add(m[1]);
    for (const m of text.matchAll(/\bicon:\s*'([a-z-]+)'/g)) asked.add(m[1]);
  }
...
test('every module import resolves to a file that exists', () => {
  for (const file of byExtension('.js')) {
    ...
    for (const match of text.matchAll(/from\s+'(\.[^']+)'/g)) {
      ...
      assert.ok(existsSync(target), `${name(file)} imports ${match[1]}, which does not exist`);
    }
  }
});
...
test('every asset a page links to ships in the package', () => {
  for (const file of byExtension('.html')) {
    ...
    for (const match of text.matchAll(/(?:src|href)\s*=\s*"([^"#][^"]*)"/g)) {
      ...
      assert.ok(existsSync(target), `${name(file)} links to ${match[1]}, which does not exist`);
    }
  }
});
...
test('every chime the player knows about actually ships', () => {
  ...
  const referenced = [...player.matchAll(/'(sounds\/[a-z-]+\.wav)'/g)].map((m) => m[1]);
  ...
  for (const file of referenced) {
    assert.ok(existsSync(join(ROOT, file)), `the player references a missing ${file}`);
  }
});

test('the manifest version is a plain, well-formed version string', () => {
  assert.match(manifest.version, /^\d+(\.\d+){0,3}$/);
  assert.ok(manifest.description.length <= 132, 'the Chrome Web Store truncates past 132 characters');
});
