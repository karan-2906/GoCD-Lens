...
test('unattended checking is off until it is explicitly asked for', () => {
  ...
  assert.equal(store.DEFAULT_SETTINGS.backgroundMinutes, 0, 'never, until asked for');
});
...
test('settings come back merged over the defaults', async () => {
  ...
  assert.equal(merged.pollSeconds, 10, 'an earlier patch is not lost by a later one');
  assert.equal(merged.theme, 'dark');
  assert.equal(merged.notifyStarred, true, 'untouched defaults survive a partial patch');
});
...
test('a settings key added in a later version defaults rather than reading undefined', async () => {
  ...
  assert.equal(settings.notifyStarred, true, 'a key the old build never wrote falls back to its default');
...
test('recent pipelines are most-recent-first, de-duplicated and bounded', async () => {
  ...
  assert.deepEqual(await store.getRecent(), ['a', 'c', 'b'], 'revisiting moves it to the front');
...
test('a redacted connection tells a page where it points, never the secret', () => {
  ...
  assert.ok(!serialised.includes('super-secret-token'), `the token leaked: ${serialised}`);
  assert.ok(redacted.secretHint.endsWith('abcd'), 'the last four are kept so two tokens can be told apart');
...
test('session mode stores no secret and says so', () => {
  ...
  assert.equal(redacted.username, undefined, 'session mode has no username to show');
});
...
test('disconnecting drops the credential and the cache but keeps stars', async () => {
  await store.setConnection({ serverUrl: 'https://gocd.example.com/go', authMode: 'token', token: 't' });
