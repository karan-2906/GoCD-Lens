...
const { state, navigate, goBack, backTarget, resetToList, selectView } = await import(
...
test('reopening the pipeline already on screen does not stack it on itself', () => {
  ...
  goBack();
  assert.deepEqual(state.route, list, 'back would otherwise land where it started');
});
...
test('the pipeline page has no hardcoded route back to the list', async () => {
  const src = await readFile(new URL('../src/dashboard/pipeline-view.js', import.meta.url), 'utf8');
  assert.ok(!/navigate\(\{\s*kind:\s*'list'/.test(src), 'back on this screen goes through goBack()');
});
