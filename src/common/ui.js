...
export function el(tag, props = {}, ...children) {
  ...
  for (const [key, value] of Object.entries(props || {})) {
    ...
    if (key === "class") node.className = value;
      ...
      throw new Error("el() does not accept raw HTML by design");
...
export async function send(type, payload = {}) {
  ...
  try {
    ...
    throw new Error(
      `The extension's background worker is not responding (${err.message}).`,
    );
  }
...
export function highlighted(text, positions) {
  ...
  const flush = () => {
    if (!run) return;
    fragment.append(
      runIsHit ? el("mark", { text: run }) : document.createTextNode(run),
    );
    run = "";
  };
