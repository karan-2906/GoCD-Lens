...
import { $, el, icon, clear, send, toast, applyTheme, loadSprite, confirmDialog } from '../common/ui.js';
import { originPattern } from '../lib/gocd.js';
import { DEFAULT_SETTINGS as DEFAULTS, backgroundPeriodMinutes } from '../lib/store.js';
...
function selectedAuthMode() {
  return document.querySelector('input[name="auth"]:checked')?.value || 'session';
}
...
function wireConnection() {
  ...
  $('#toggle-token').addEventListener('click', () => toggleSecret('#token', '#toggle-token'));

  $('#save').addEventListener('click', () => withBusy($('#save'), save));
  $('#test').addEventListener('click', () => withBusy($('#test'), test));
  $('#disconnect').addEventListener('click', disconnect);
  $('#run-diagnostics').addEventListener('click', () => withBusy($('#run-diagnostics'), diagnose));
}
...
function warnAboutUrl() {
  ...
  if (url.protocol === 'http:' && !isLocal) {
    ...
    host.append(banner('error', 'alert', 'That does not look like a web address.'));
  } else if (!/\/go\/?$/.test(url.pathname) && url.pathname.replace(/\/+$/, '') === '') {
    host.append(
      banner(
        'info',
        'info',
...
function readForm() {
  ...
  try {
    ...
    throw new Error(`"${serverUrl}" is not a web address. It should look like https://gocd.example.com/go`);
  }
  ...
  if (authMode === 'token') {
    ...
    if (!token && !(state.connection?.authMode === 'token' && state.connection?.hasSecret)) {
      throw new Error('Paste your GoCD access token, or switch to using your existing login.');
    }
    ...
    else if (state.connection?.authMode === 'basic' && state.connection?.hasSecret) {
...
async function test() {
  ...
  if (connection.keepExistingSecret) {
    throw new Error('Testing a saved secret is not possible from here - press Save to verify it.');
  }
  await ensurePermission(connection.serverUrl);
...
function showResult(tone, message) {
  ...
  host.append(banner(tone === 'ok' ? 'info' : tone, tone === 'ok' ? 'check' : 'alert', message));
}

function banner(tone, iconName, message) {
  return el('div', { class: `banner banner-${tone}` }, icon(iconName), el('span', { text: message }));
}
...
function paintSettings() {
  ...
  selectValue('#background-minutes', s.backgroundMinutes, DEFAULTS.backgroundMinutes);
...
function wireSettings() {
  ...
  $('#poll').addEventListener('change', patch(() => ({ pollSeconds: Number($('#poll').value) })));
  ...
  $('#background-minutes').addEventListener(
    'change',
    patch(() => ({ backgroundMinutes: Number($('#background-minutes').value) })),
  );
  ...
  $('#notify-starred').addEventListener('change', patch(() => ({ notifyStarred: $('#notify-starred').checked })));
  ...
  $('#theme').addEventListener('change', patch(() => ({ theme: $('#theme').value })));
  $('#density').addEventListener('change', patch(() => ({ density: $('#density').value })));
...
function paintLoadEstimate() {
  ...
  list.append(
    ...lines.map((line) =>
      el('li', {}, icon('activity'), el('span', { class: 'muted', text: line })),
    ),
  );
  host.append(
    el('h3', { class: 'subhead', style: { marginTop: '4px' } }, 'How this affects GoCD'),
    list,
  );
  ...
  const changed = lines.filter((line, at) => previousCostLines[at] !== undefined && previousCostLines[at] !== line);
...
function paintNotificationDelay() {
  ...
  const lead = 'Notifications are checked periodically and may be delayed by the configured ';
  const tail = '.';

  host.append(
    el('div', { class: 'banner banner-info' }, icon('clock'), el('span', {}, lead, where, tail)),
  );
}
...
function browserIdentity() {
  ...
  const named = brands.find(
    (brand) => !/not.*brand/i.test(brand.brand) && !/^chromium$/i.test(brand.brand),
  );
  ...
  if (named?.brand === 'Brave' || navigator.brave) return { name: 'Brave', scheme: 'brave' };
  if (named?.brand === 'Opera' || /\bOPR\//.test(ua)) return { name: 'Opera', scheme: 'opera' };
...
function osNotificationPath() {
  ...
  if (/mac/i.test(platform)) return `System Settings \u2192 Notifications \u2192 ${name}`;
  if (/win/i.test(platform)) return `Settings \u2192 System \u2192 Notifications \u2192 ${name}`;
    ...
    el('button', {
      ...
      onclick: async () => {
        try {
          ...
          toast(`Sent. If nothing appeared, check ${path}.`, { tone: 'info', timeout: 9000 });
        } catch (err) {
...
async function preview(name) {
  try {
    ...
    if (result && !result.ok) {
      toast(`Your browser would not play it: ${result.error}`, { tone: 'warn', timeout: 7000 });
    }
  } catch (err) {
    toast(err.message, { tone: 'error' });
  }
  ...
  for (const [title, detail] of facts) {
    list.append(
      el(
        ...
        {},
        icon('shield'),
        el('span', {}, el('strong', { text: title }), ' ', el('span', { class: 'muted', text: detail })),
      ),
    );
  }
}
