...
export function renderList(host) {
  ...
  if (starred.length && state.filter !== 'favorites') {
    host.append(sectionHeader({ name: 'Starred', matches: starred, pinned: true }));
  ...
  if (sections.length === 1) {
    if (!state.group) host.append(sectionHeader({ ...sections[0], alwaysOpen: true }));
...
function collapseAllBar(sections) {
  const allCollapsed = sections.every((section) => isGroupCollapsed(section.name));
  return el(
    'div',
    { class: 'list-toolbar' },
    el('span', {
...
function sectionHeader({ name, matches, pinned = false, alwaysOpen = false }) {
  const failing = matches.filter((m) => pipelineStatus(m.pipeline) === 'Failed').length;
  const building = matches.filter((m) => pipelineStatus(m.pipeline) === 'Building').length;
  ...
  const contents = [
    ...
    failing > 0 && el('span', { class: 'section-tag tone-fail', text: `${failing} failing` }),
    building > 0 && el('span', { class: 'section-tag tone-build', text: `${building} running` }),
  ];
...
function card({ pipeline, hits, viaGroup }) {
  ...
  const actions = el(
    ...
    iconAction(
      ...
      watching ? 'Stop notifying me about this' : 'Notify me when this starts and finishes',
    ...
    iconAction(paused ? 'play' : 'pause', paused ? 'Resume' : 'Pause', () => togglePause(pipeline)),
  );
  ...
  const body = el(
    ...
    el(
      ...
      { class: 'card-meta' },
      statusPill(status),
      watching && el('span', { class: 'watch-flag', title: 'You are being notified about this one' }, icon('bell-filled', { size: 11 })),
      paused && el('span', { class: 'paused-flag' }, icon('pause', { size: 12 }), 'Paused'),
...
function iconAction(name, title, onClick, active = false, activeClass = 'starred') {
  return el(
    'button',
    {
...
function recentStrip() {
  ...
  const recents = (state.recent || []).filter((name) => known.has(name)).slice(0, RECENT_SHOWN);
  ...
  for (const name of recents) {
    ...
    strip.append(
      el(
        ...
        el('span', { class: `recent-dot tone-${tone(pipelineStatus(pipeline))}` }),
...
function knownNames(names) {
  ...
  for (const name of names) {
    panel.append(
      el(
        'button',
        { class: 'name-row', onclick: () => navigate({ kind: 'pipeline', name }) },
        icon('layers', { size: 13 }),
        el('span', { class: 'truncate', text: name }),
