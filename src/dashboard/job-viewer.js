...
function setWrap(on) {
  ...
  if (button) {
    ...
    clear(button).append(
      icon(on ? "check" : "list", { size: 12 }),
      document.createTextNode("Wrap"),
    );
  }
...
function setFollow(on) {
  ...
  if (button) {
    button.classList.toggle("toggle-on", on);
    clear(button).append(
      icon(on ? "check" : "arrow-down", { size: 12 }),
  ...
  if (session.tab === "console") {
    ...
    bar.append(
      el(
        "div",
        { class: "search-wrap" },
        el("span", { class: "search-icon" }, icon("search", { size: 14 })),
        search,
      ),
      el(
        "button",
      ...
      el(
        "button",
        { class: "icon-btn", title: "Next match", onclick: () => stepMatch(1) },
        icon("chevron-right"),
      ),
      ...
      el(
        "button",
        {
          ...
          onclick: () => copyToClipboard(session.lines.join("\n")),
        },
        icon("copy", { size: 12 }),
      ),
    );
    ...
    bar.append(
      el("span", {
        class: "faint",
        text: "Files this job published. Clicking one opens it in a new tab.",
      }),
      el("span", { class: "spacer" }),
      el(
        "button",
        { class: "btn btn-sm", onclick: loadArtifacts },
  ...
  bar.append(
    el(
      "button",
      {
        ...
        onclick: () =>
          openInGoCd("job", { pipeline, counter, stage, stageCounter, job }),
      },
...
async function loadConsole({ initial = false } = {}) {
  ...
  try {
    const { text } = await send("consoleLog", {
      pipeline,
      counter,
      stage,
      stageCounter,
    ...
    session.error =
      err.status === 404 && session.lines.length === 0
...
function startTail() {
  session.timer = setInterval(async () => {
    ...
    if (session.lines.length === before)
      session.idleTicks = (session.idleTicks || 0) + 1;
...
async function loadArtifacts() {
  ...
  renderBody();
  try {
    session.artifacts = await send("artifacts", {
      pipeline,
      counter,
      stage,
      stageCounter,
  ...
  if (session.loading) {
    body.append(
      el(
        ...
        el("p", { text: "Fetching console output..." }),
      ),
    );
    return;
  }
  ...
  if (session.lines.length === 0) {
    body.append(
      el(
        ...
        el("p", {
          text: session.error || "This job has not printed anything.",
        }),
      ),
    );
    return;
  ...
  if (start > 0) {
    log.append(
      el(
        ...
        el("span", {
          class: "log-text faint",
          text: `... ${start} earlier lines not shown -- use Download for the full log ...`,
        }),
      ),
    );
  }
...
function stepMatch(direction) {
  ...
  session.matchIndex =
    (session.matchIndex + direction + session.matchNodes.length) %
    session.matchNodes.length;
  ...
  if (rows.length === 0) {
    body.append(
      el(
        ...
        { class: "empty" },
        icon("folder", { size: 26 }),
        el("h3", { text: "No artifacts" }),
        el("p", { text: "This job did not publish any files." }),
      ),
    );
    return;
  }
    ...
    list.append(
      el(
        "button",
        {
          ...
          onclick: () => {
            if (row.isFolder) {
              if (session.expanded.has(row.path))
                session.expanded.delete(row.path);
...
function paintFooter() {
  ...
  if (session.error)
    ...
    foot.append(
      el("span", { class: "spacer" }),
      el("span", { class: "follow-on", text: "Live - checking every 3s" }),
    );
  else if (session.finished)
    foot.append(
      el("span", { class: "spacer" }),
      el("span", { text: "Job finished" }),
    );
}
...
function downloadLog() {
  ...
  const link = el("a", {
    href: url,
    download: `${pipeline}-${counter}-${stage}-${job}.log`,
  });
  link.click();
